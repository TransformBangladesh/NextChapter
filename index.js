...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderPomodoroSettings = () => {
        $('#setting-focus-time').value = String(state.pomodoro.settings.focus);
        $('#setting-short-break-time').value = String(state.pomodoro.settings.short);
        $('#setting-long-break-time').value = String(state.pomodoro.settings.long);
    ...
    const finishRoomSession = async (skipped = false) => {
        const durationSeconds = state.pomodoro.settings[state.room.mode] * 60 - (skipped ? state.room.timeRemaining : 0);
    ...
    const getTotalStudyMinutes = (currentState) => {
        return currentState.sessions.reduce((acc, s) => acc + s.duration, 0);
    };
    ...
    const ACHIEVEMENTS_LIST = {
        'first_session': { name: 'First Step', description: 'Complete your first study session.', icon: '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>', condition: (s) => s.sessions.length >= 1 },
        'one_hour': { name: 'Focused Hour', description: 'Log 1 hour of total study time.', icon: '<path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z"/><path d="M12.5 7H11v6l5.25 3.15.75-1.23-4.5-2.67z"/>', condition: (s) => getTotalStudyMinutes(s) >= 60 },
        'ten_hours': { name: 'Dedicated Learner', description: 'Log 10 hours of total study time.', icon: '<path d="M21 5.47L12 12 3 5.47V18h18V5.47zM21 3H3c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z"/>', condition: (s) => getTotalStudyMinutes(s) >= 600 },
        'streak_3': { name: 'On a Roll', description: 'Maintain a 3-day study streak.', icon: '<path d="M16.5 12c-1.38 0-2.5 1.12-2.5 2.5s1.12 2.5 2.5 2.5 2.5-1.12 2.5-2.5-1.12-2.5-2.5-2.5zM9 12c-1.38 0-2.5 1.12-2.5 2.5s1.12 2.5 2.5 2.5 2.5-1.12 2.5-2.5S10.38 12 9 12zm2.14-5.54c.73-.42 1.58-.66 2.5-.66.92 0 1.77.24 2.5.66l1.45-2.29C16.36 2.91 14.76 2 13 2c-1.76 0-3.36.91-4.59 2.17l1.73 1.29z"/>', condition: (s) => s.streak.current >= 3 },
        'streak_7': { name: 'Week of Focus', description: 'Maintain a 7-day study streak.', icon: '<path d="M21.99 4c0-1.1-.89-2-1.99-2H4c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V4zm-2 14H4V8h16v10z"/>', condition: (s) => s.streak.current >= 7 },
        'first_note': { name: 'Note Taker', description: 'Create your first note.', icon: '<path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/>', condition: (s) => s.notes.length > 0 },
    };
    ...
    const setupEventListeners = () => {
        document.body.addEventListener('click', async (e) => {
            ...
            if(target.closest('#add-subject-btn-header')) $('#add-subject-btn').click();
            if(target.closest('#new-note-btn-header')) $('#new-note-btn').click();
